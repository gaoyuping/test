...
#include<iostream>
using namespace std;
VideoPlayer::VideoPlayer(QString url)
{
    strUrl = url;
}
...
void VideoPlayer::run()
{
    ...
    QByteArray ba = strUrl.toLocal8Bit();
    char *url = ba.data();
    ...
    {
        iframe_rate = pCodecCtx->time_base.den / pCodecCtx->time_base.num;
    }
    ...
    numBytes = avpicture_get_size(AV_PIX_FMT_BGRA, pCodecCtx->width, pCodecCtx->height);

    out_buffer = (uint8_t *) av_malloc(numBytes * sizeof(uint8_t));
    avpicture_fill((AVPicture *)pFrameRGB, out_buffer, AV_PIX_FMT_BGRA,
...
//                emit sig_GetRFrame(image);
     ...
     ///2017.8.7---lizhen
        msleep(1000 / iframe_rate); //停一停  不然放的太快了
    }
    av_free(out_buffer);
    av_free(pFrameRGB);
